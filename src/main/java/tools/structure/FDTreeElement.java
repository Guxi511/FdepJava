...
public class FDTreeElement {
     ...
     * false otherwise.
     */
    // 因为找指定fd的generalization，只需要找到lhs的子集指向a就行了。所以分两个分支走，一个是以当前lhs的第一位作为路径开始进行向下，如果没有再对下一位进行
    public boolean containsGeneralization(BitSet lhs, int a, int currentAttr) {
        ...
        // 因为found = False时会进行上面一句操作，所以到return true这一部时found = True，其实就是返回found
		return true;
    }
...
//		}
        ...
        // 如果nextSetAttr = -1 代表 lhs路径走完了，但要找的是lhs的specialization，所以继续往深入找
        if (nextSetAttr < 0) {
            //
